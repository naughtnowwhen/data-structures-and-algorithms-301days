...
let appendTheEnd = function (str) {
  let appendMe = 'The end.'; //?
  str = str + " " + appendMe; //?
  return str;
};
appendTheEnd('yep');
...
let appendFirstToLast = function (arr) {
  let appendME = arr[0];
  arr.push(appendME); //?
  arr; //?
};
appendFirstToLast([5, 2, 3, 4, 5, 6]);
...
let testObj = { fullName: 'jones grant' };
let addBirthYearProperty = function (obj, year) {
    obj.yearBorn = year;
    obj; //?
};
addBirthYearProperty(testObj, 1456);
...
let setStatusAsAuthor = function (people) {
