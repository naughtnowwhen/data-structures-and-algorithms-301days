...
------------------------------------------------------------------------------------------------ */
let shortenMe = 'ibeginlongbutthelongeriruntheshorteriget';

const howMuchPencil = (str) => {
  let result = [];
  let splitToArray = str.split('');
  splitToArray.forEach((letter, index, arr) => {
    let sliced = arr.slice(index, arr.length);
...
howMuchPencil(shortenMe); //?
let testString = 'imATestSplice';
let converToArr = testString.split(''); //?
converToArr.slice(5, converToArr.length); //?
...
const listFoods = (recipe) => {
  let result = [];
  let justrecipe = recipe.ingredients;
  let regex = /\d\s\w+(-?\w+)\s/;

  let first = justrecipe[1]; //?
  let slicer = first.match(regex)[0]; //?
  slicer.length //?
  let sliced = first.slice(slicer.length, first.length) //?

  justrecipe.forEach((rec)=>{
    let recSlicer = rec.match(regex)[0];
    let recSliced = rec.slice(recSlicer.length, rec.length); //?
  })
...
const splitFoods = (recipe) => {
  ...
  // let first = justRecipe[0]; //?
  // let split = first.split(' '); //?
  // let sliced = split.slice(2, split.length) //?


  justRecipe.forEach((ing)=>{
    let split = ing.split(' ');
    let sliced = split.slice(2, split.length); //?
    result.push(sliced); 
  })
...
const stepActions = (recipe) => {
  let result = [];
  let directions = recipe.steps; //?
  directions.forEach((step)=>{
    let split = step.split(' '); //?
    let slice = split.slice(0,1)[0]; //?
    result.push(slice);
  })
  return result;
}
...
let expected =  ["Pre-heat", "De-prickle", "Sprinkle", "Mix", "Grease", "Combine", "Fold", "Spread", "Bake"];
...
removeLastCharacters(removeTest, 123); //?
