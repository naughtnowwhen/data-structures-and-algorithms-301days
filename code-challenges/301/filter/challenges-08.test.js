...
------------------------------------------------------------------------------------------------ */
let inputBothReturnOdd = [1, 2, 3, 4, 5, 6, 7, 8, 9];

const oddValues = (arr) => {
  let returnMe = arr.filter(val => val % 2 != 0);
  return returnMe;
};

oddValues(inputBothReturnOdd); //?
...
// borrowed code to assemble my input arr;
// let alphabet = 'abcdefghijklmnopqrstuvwxyz'.split(''); 
let someWords = ['apple', 'cv', 'st', 'xx', 'a', 'b', 'x', 'e'];
...
const filterStringsWithVowels = (arr) => {
  ...
  let mapLettersThatIncludVowels = arr.map((letter) => letter.includes(letter.match(regex)));
  console.log(mapLettersThatIncludVowels);

  let returnMyVowels = arr.filter((letter) => {}) //?

};

filterStringsWithVowels(someWords);
...
let forbiddenCheck = [90, 91, 92, 93, 94, 95];
let baselineCheck = [1, 2, 3, 4, 5, 6, 7, 8, 9, 93];

const notInFirstArray = (forbiddenValues, arr) => {
  let filterOut = arr.filter((val) => {
    return !forbiddenValues.includes(val);
  })
  return filterOut; //?
};

notInFirstArray(forbiddenCheck, baselineCheck); //?
...
//   let drillToArrayInOrderToMap = arr.stats;
//   drillToArrayInOrderToMap instanceof Array; //?

//   let mappedToBaseline = drillToArrayInOrderToMap.map((creatureObj) => creatureObj.baseStat);
//   let filterOutTheSmallUnderThreshold = mappedToBaseline.filter((baseState, ind, array) => {
//     drillToArrayInOrderToMap; //?    
//     return baseState > minBaseStat ? drillToArrayInOrderToMap[ind] : false;
//   })
//   return filterOutTheSmallUnderThreshold; 
// };

// getBaseStatGreaterThan(snorlaxData, 50);  //?
...
const getStatName = (arr, minBaseStat) => {
  ...
  let goFromGeneralToSpecific = filtered.map((obj) => {
    return obj.stat.name;
  })
  return goFromGeneralToSpecific;
};
...
const getCharactersWithoutChildren = (arr) => {
  let returnTheChildless = arr.filter((characterObj) => {
    return characterObj.children == undefined ? true : false;
  })
  return returnTheChildless; //?
};
...
let inputWithVariation = [1, 2, 3, true, false, 'grouch', 'fun', 'dancing', 'shame'];
...
const evenOddNumericValues = (arr) => {
  ...
  let stringifyLogic = filtered.map(val =>{
    if (val % 2 == 0){
      return 'even';
    }
    else {
  ...
  return stringifyLogic;
};

evenOddNumericValues(inputWithVariation); //?
