...
let countInputter = [[1, 3, 5, 7, 9], [5, 5, 5], [1, 2, 3]];

const count = (target, input) => {
    let altered = input.reduce((thing,val,index)=>{
       if (val.includes(target)){
         let filtered =  val.filter((num)=>{
               if (num = target && thing instanceof Array){
                   num;
                   thing.push(num);
               }
           })
