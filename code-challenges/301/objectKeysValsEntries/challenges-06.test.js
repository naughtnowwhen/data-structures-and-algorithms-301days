...
// character; //?  
//   let bigList = Object.values(arr); //?
//   let pushAll = [];
//   bigList.forEach((charac)=>{
//     if(charac.children.length > 0){
//       pushAll.push(true);
//     }
//     else pushAll.push(false);
//   })
...
const hasChildrenValues = (arr, character) => {
  ...
  arr.forEach((charac) => {
    if (charac.name === character) {
      character; //?
      charac.children.length; //?
      myTestObj.charac = {};
      if (charac.children.length > 0) {

        myTestObj.charac.name = charac.name;
        myTestObj.charac.hasChildren = true;
        myTestObj.charac.hasChildren; 
        myTestObj; //?
      } else if ((charac.children.length == 0)) myTestObj.charac.name = charac.name;
        myTestObj.charac.hasChildren = false;
    }
  })
  myTestObj; //?

  if (myTestObj.charac.hasChildren == true) {
    console.log('anytghing')
    return true;
  }
  else if (myTestObj.charac.hasChildren == false){
    console.log('what')
    return false;
  }
...
hasChildrenValues(characters, "Eddard"); //?
...
const hasChildrenEntries = (arr, character) => {
  arr.forEach(value => {
    ...
    if (charObj[0][1] === character) {
      console.log(charObj[0][1])
      if (charObj[2][1].length > 0) {
        console.log(charObj[2][1].length)
        return (true);
      } else {
        return (false)
      }
...
hasChildrenEntries(characters, "Eddard"); //?
