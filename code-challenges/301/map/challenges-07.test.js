...
------------------------------------------------------------------------------------------------ */
let powerMe = [2, 3, 4, 5, 6, 7];

const forLoopTwoToThe = (arr) => {
  let outputter = [];
  for (let num of arr) {
    num; //?
    outputter.push(Math.pow(2, num));
  }
  return outputter; //?
};

// forLoopTwoToThe(powerMe);
...
------------------------------------------------------------------------------------------------ */
let powerMeForEach = [2, 3, 4, 5, 6, 7, ];

const forEachTwoToThe = (arr) => {
  let returnMe = [];
  arr.forEach((num) => {
    returnMe.push(Math.pow(2, num));
  })
  return returnMe;
};
// forEachTwoToThe(powerMeForEach) //?
...
const mapTwoToThe = (arr) => {
  let powered = arr.map(x => Math.pow(2, x));
  return powered;
}
...
let inputterCharCode = ['a', 'b', 'c'];

const charCode = (arr) => {
  var map = Array.prototype.map;

  let mapped = map.call(arr, (x) => {
    return x.charCodeAt(0); //?
  })
  return mapped;
};

// let returner = charCode(inputterCharCode); //?
...
------------------------------------------------------------------------------------------------ */
let inputEvenOdd = [1, 2, 3, true, false];
...
const evenOdd = (arr) => {
  let mapped = arr.map((ele, ind, array) => {
    typeof ele; //?
    array; //?
    if(typeof ele != 'number') {
      ele = 'N/A'; //?
...
const extractAbilities = (arr) => {
  let arrIWantTOTarget = arr.abilities; //?
  let mapped = arrIWantTOTarget.map((thing)=>{
    return thing.ability.name;
  })
return mapped;
};

extractAbilities(snorlaxAbilities); //?
...
const extractStats = (arr) => {
  let iterateMe = arr.stats; 
  let mapped = iterateMe.map((thing) => {
    return {
      name : thing.stat.name,
      total : thing.baseStat + thing.effort,
    }
  }) 
  return mapped;
};

extractStats(snorlaxStats); //?
